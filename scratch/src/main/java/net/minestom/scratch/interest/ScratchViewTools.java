...
import static net.minestom.scratch.network.ScratchNetworkTools.NetworkContext.Packet;
...
public final class ScratchViewTools {
    ...
    public static final class Synchronizer {
        ...
        public void signalAt(int chunkX, int chunkZ, ServerPacket.Play packet) {
            Chunk chunk = chunks.get(CoordConversionUtils.chunkIndex(chunkX, chunkZ));
            if (chunk != null) chunk.broadcaster.append(packet);
        }

        public void computePackets(BiConsumer<Integer, Packet> consumer) {
            // Update chunks viewers
            for (int entryId : entriesChanged) {
                Entry entry = entries.get(entryId);
                if (entry == null) continue;
                if (entry.initialized) {
                    final long oldChunkIndex = CoordConversionUtils.chunkIndex(entry.oldChunkX, entry.oldChunkZ);
                ...
                if (entry.alive) {
                    final long newChunkIndex = CoordConversionUtils.chunkIndex(entry.newChunkX, entry.newChunkZ);
            ...
            for (int entryId : entriesChanged) {
                ...
                ChunkRangeUtils.ChunkConsumer newCallback = (x, z) -> {
                    Chunk chunk = chunks.computeIfAbsent(CoordConversionUtils.chunkIndex(x, z), Chunk::new);
                    for (int viewerId : entry.receiver ? chunk.viewers : chunk.viewersReceivers) {
                        ...
                        if (entry.receiver) {
                            ...
                            consumer.accept(entryId, new Packet.PlayList(packets));
                        }
                        if (viewer.receiver) {
                            final List<ServerPacket.Play> packets = entry.initSupplier.get();
                            consumer.accept(viewerId, new Packet.PlayList(packets));
                        }
                    }
                    if (entry.receiver) chunk.receivers.add(entryId);
                };
                ChunkRangeUtils.ChunkConsumer oldCallback = (x, z) -> {
                    final Chunk chunk = chunks.get(CoordConversionUtils.chunkIndex(x, z));
            ...
            for (Chunk chunk : chunks.values()) {
                try (Broadcaster.Collector collector = chunk.broadcaster.collector()) {
                    ...
                    for (int viewerId : chunk.receivers) {
                        ...
                        consumer.accept(viewerId, new Packet.PlayList(packets, exception));
                    }
                }
            }
