...
public final class SurvivalTemplate {
    ...
    private final Broadcaster serverBroadcast = new Broadcaster();
    ...
    private final List<Recipe> recipes = List.of(
            new Recipe.Shaped("minecraft:stick", "group",
            ...
            new Recipe.Shaped("minecraft:crafting_table", "group",
                    RecipeCategory.Crafting.MISC,
                    2, 2,
                    List.of(
                            new Recipe.Ingredient(ItemStack.of(Material.OAK_PLANKS)),
            ...
            new Recipe.Shaped("minecraft:furnace", "group",
                    RecipeCategory.Crafting.MISC,
                    3, 3,
                    List.of(
                            ...
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)),
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)),
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)), // FIXME: should be air?
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)),
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)),
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE)),
                            new Recipe.Ingredient(ItemStack.of(Material.COBBLESTONE))
    ...
    void ticks() {
        ...
        while (!stop.get()) {
            ...
            // Compute global broadcast packets
            try (Broadcaster.Collector collector = serverBroadcast.collector()) {
            ...
            for (Instance world : worlds) {
                ...
                world.synchronizer.computePackets((playerId, packet) -> {
                    ...
                    if (player != null && player.instance == world) player.connection.networkContext.write(packet);
                });
            }
            {
    ...
    final class ItemEntity {
        ...
        ItemEntity(Instance instance, Pos position, ItemStack itemStack) {
            ...
            this.synchronizerEntry = instance.synchronizer.makeEntry(false, id, position, initPackets, destroyPackets);
        ...
        boolean tick() {
            ...
            if (!velocity.isZero()) synchronizerEntry.signal(new EntityVelocityPacket(id, velocity.mul(8000f)));
            ...
            if (tickAlive >= PICKUP_DELAY) {
                for (Player player : players.values()) {
                    if (player.position.distance(position) < type.registry().boundingBox().width() + 1) {
                        ...
                        this.synchronizerEntry.signal(new CollectItemPacket(id, player.id, itemStack.amount()));
                        return false;
                    }
                }
            }
    ...
    final class Player {
        ...
        final BlockInteractionHandler blockInteractionHandler = new BlockInteractionHandler(id, this::sendPacket, play -> synchronizerEntry.signal(play));
        final MetaHolder metaHolder = new MetaHolder(id, play -> {
            sendPacket(play);
            this.synchronizerEntry.signal(play);
        });
        final HealthHandler healthHandler = new HealthHandler(id, this::sendPacket, play -> synchronizerEntry.signal(play));
        final InventoryHolder inventoryHolder = new InventoryHolder(id, this::sendPacket, play -> synchronizerEntry.signal(play));
        ...
        final LegacyStringArrayCommands commands = new LegacyStringArrayCommands(
                Map.of(
                        ...
                        "say", args -> serverBroadcast.append(new SystemChatPacket(Component.text("[" + Player.this.username + "] ")
        ...
        Player(PlayerInfo info, Instance spawnInstance, Pos spawnPosition) {
            ...
            this.synchronizerEntry = instance.synchronizer.makeEntry(true, id, position, initPackets, destroyPackets);
            ...
            serverBroadcast.append(getAddPlayerToList());
            for (Player player : players.values()) {
        ...
        private void teleport(Pos target) {
            ...
            this.synchronizerEntry.signal(new EntityTeleportPacket(id, target, false));
            ...
            this.connection.packetQueue.drain(packet -> {
                ...
                switch (packet) {
                    ...
                    case ClientAnimationPacket animationPacket -> {
                        ...
                        this.synchronizerEntry.signal(new EntityAnimationPacket(id, animation));
                    }
                    case ClientPlayerDiggingPacket diggingPacket -> {
                        ...
                        if (diggingPacket.status() == ClientPlayerDiggingPacket.Status.UPDATE_ITEM_STATE) {
                            ...
                            if (updated.isAir()) synchronizerEntry.signal(inventoryHolder.equipmentPacket());
                            ...
                            synchronizerEntry.signal(inventoryHolder.equipmentPacket());
                        }
                    }
